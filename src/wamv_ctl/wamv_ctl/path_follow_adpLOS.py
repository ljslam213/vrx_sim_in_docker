...
class EnhancedPathTrackingController(Node):
    def __init__(self):
        ...
        self.converge_dist = 3.0  # 降低收敛阈值
        self.max_angular_rate = math.radians(45)  # 降低最大角速度
        self.smooth_factor = 0.3   # 指令平滑系数
        self.beta_hat = 0.0      # 侧滑角估计初值
        self.gamma = 0.2         # 自适应增益（0.1~1.0）
        ...
        self.pid_wz = PIDController(kP=0.4, kI=0.02, kD=0.5)  

        # 状态变量
        self.cur_pos = None
        self.cur_rot = None
        self.path_points = []
        self.path_tangents = [] ## 存储每个路径点的切线向量
    ...
    def adaptive_lookahead(self, cross_track_error, speed=1.0):
        ...
        # 基础前视距离考虑速度和曲率
        base_lookahead = self.min_lookahead + 2.5 * speed ## 与速度成正比
        base_lookahead *= 1.0 / (1.0 + 8.0 * path_curvature) ## 与曲率成反比
    ...
    def los_guidance(self, lookahead_point, tangent):
        ...
        # 自适应侧滑角更新（论文式29）
        gamma = 0.1  # 自适应增益
        U = max(0.5, abs(self.last_cmd.linear.x))  # 设置速度下限
        ...
        beta_delta = np.clip(beta_delta, -math.radians(5), math.radians(5))  # 限制变化率
        self.beta_hat = self.normalize_angle(self.beta_hat + beta_delta)
        self.beta_hat = np.clip(self.beta_hat, -math.radians(30), math.radians(30))  # 限制范围
    ...
    def control_loop(self):
        ...
        speed_limit *= 1.0 / (1.0 + 8.0 * path_curvature + 2.0 * abs(cross_track_error))  # 高曲率或高误差时减速
        ...
        # 侧滑角过大时自动降速
        if abs(self.beta_hat) > math.radians(25):
            forward_speed *= 0.7
            angular_rate *= 0.8
        ...
        self.get_logger().info(
            ...
            f"状态: 侧滑角={math.degrees(self.beta_hat):.1f}° 前视距={self.lookahead_dist:.1f}m\n"
